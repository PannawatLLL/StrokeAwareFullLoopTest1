...
const MotorWeaknessAssessment = ({ onComplete }) => {
  // Set this to true if you want the mirrored webcam preview.
  // If you set it to false, you can remove the flipping logic below.
  ...
  const [holdDuration] = useState(10); // Need to hold each side for 10 seconds
  ...
  useEffect(() => {
    const loadModels = async () => {
      ...
      const handDetectorInstance = await handPoseDetection.createDetector(handModel, handDetectorConfig);
      ...
      const poseDetectorInstance = await poseDetection.createDetector(poseModel, poseDetectorConfig);
      
      setHandDetector(handDetectorInstance);
      setPoseDetector(poseDetectorInstance);
      setIsLoading(false);
    };
    ...
    return () => {
      // Dispose if you like. (MediaPipe doesn't fully support disposal in all builds)
    ...
    const detect = async () => {
      if (webcamRef.current?.video?.readyState === 4) {
        ...
        if (posePredictions.length > 0) {
          ...
          // If using mirrored feed, flip the shoulders' x to get correct geometry
          if (isMirrored) {
            if (shoulders.left && shoulders.left.score > 0.3) {
              shoulders.left.x = videoWidth - shoulders.left.x;
            }
            if (shoulders.right && shoulders.right.score > 0.3) {
              shoulders.right.x = videoWidth - shoulders.right.x;
            }
          }
          ...
          [shoulders.left, shoulders.right].forEach((shoulder, index) => {
            if (shoulder && shoulder.score > 0.3) {
              const isLeftShoulder = (index === 0);
              const color = isLeftShoulder ? colors.left : colors.right;
              ...
              // For demonstration, let's keep the target up and horizontally outward:
              const targetX = isLeftShoulder 
        ...
        handPredictions.forEach((prediction) => {
          ...
          // The model says 'left' or 'right' based on real (unmirrored) camera feed
          const modelHandedness = prediction.handedness.toLowerCase();

          // Flip "left" <-> "right" if isMirrored
          const actualSide = isMirrored
            ? (modelHandedness === 'left' ? 'right' : 'left')
            : modelHandedness;

          // We'll update newHandStates for the actual side
          newHandStates[actualSide].detected = true;

          // For color-coding
          const color = (actualSide === 'left') ? colors.left : colors.right;

          // Flip x-coordinates if mirrored
          const correctedLandmarks = landmarks.map((lm) => ({
            x: isMirrored ? (videoWidth - lm.x) : lm.x,
            y: lm.y,
          }));
          ...
          const correctedWrist = correctedLandmarks[0];

          // Identify which shoulder to use
          const sideShoulder = (actualSide === 'left') ? shoulders.left : shoulders.right;
          if (sideShoulder && sideShoulder.score > 0.3) {
            // Check if shoulder is near the line
            const shoulderOnLine = Math.abs(sideShoulder.y - shoulderLineY) < 20;
            newHandStates[actualSide].shoulderTouching = shoulderOnLine;

            // Calculate angle between shoulder and wrist
            const dx = correctedWrist.x - sideShoulder.x;
            // y-axis is normal top->down, so do shoulder.y - wrist.y if you want positive angles going upward
            const dy = sideShoulder.y - correctedWrist.y;
            let angle = Math.atan2(dy, dx) * (180 / Math.PI);

            // Normalize angle to 0..360
            if (angle < 0) angle += 360;

            // For left arm, we want ~135°, for right arm ~45°
            const targetAngleForSide = (actualSide === 'left') ? 135 : 45;
            const angleDiff = Math.abs(angle - targetAngleForSide);
            
            newHandStates[actualSide].correctAngle = (angleDiff <= angleTolerance);
        ...
        // Now check if the current side meets conditions
  ...
  const getStatusMessage = () => {
    ...
    // If we've passed all checks, then we're holding the position
        ...
        <div style={{
          ...
          <div style={{
            ...
            <h2 style={{
              ...
              แบบประเมินกล้ามเนื้ออ่อนแรง
            </h2>
            <p style={{ marginBottom: '20px', fontSize: '1.3rem', color: 'red', fontFamily: "Prompt", fontWeight:"500"}}>
              โปรดอ่านก่อนเริ่มทำการประเมิน
            </p>
            <p style={{ marginBottom: '20px', fontSize: '1.1rem', color: '#5c5c5c', fontFamily: "Prompt"}}>
              การทดสอบนี้จะประเมินความแข็งแรงของแขน โดยให้คุณถือแขนแต่ละข้างเป็นมุม <span style={{color:"black",fontWeight:"600",textDecoration:"underline"}}>45 องศา</span>
            </p>
      ...
      {!isLoading && (
        <div style={{
          ...
          <Webcam
            audio={false}
            ref={webcamRef}
            // We keep the feed mirrored for user experience
            mirrored={isMirrored}
            playsinline
